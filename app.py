...
if ghost_active:
    ...
    theme_clr = "#ff3131" # Security Red
else:
    st.markdown("<style>.stApp { background-color: #0E1117; color: white; }</style>", unsafe_allow_html=True)
    theme_clr = "#3b82f6" # Professional Blue
...
st.markdown(f"""
    ...
    .stButton>button {{ background-color: #1E3A8A; color: white; border-radius: 10px; width: 100%; }}
    </style>
...
if not st.session_state.auth:
    ...
    with t2:
        ...
        if st.button("Complete Sign Up"):
            ...
            st.session_state.user = {"name": name, "role": "User" if "User" in role else "Therapist"}
            st.rerun()
    ...
    # --- USER VIEW (THE JOURNEY) ---
        ...
        if st.session_state.step == 0:
            ...
            if prompt := st.chat_input("How are you feeling?"):
                st.chat_message("assistant").write(engine.get_calming_msg(prompt))
                if st.button("Start 150-Variable Assessment"):
        ...
        # Step 1: Assessment
            ...
            s2 = st.slider("Sleep Issues", 1, 10, 5)
        ...
        # Step 2: Match & P2P Chat
        elif st.session_state.step == 2:
            doc = st.session_state.match
            st.success(f"Match Found: {doc['name']} ({doc['match']}% Match)")
            ...
            with c_chat:
                ...
                for m in st.session_state.p2p_chat:
                    st.write(f"**{m['role']}:** {m['content']}")
                st.markdown('</div>', unsafe_allow_html=True)
                if u_input := st.chat_input("Send message to therapist..."):
                    st.session_state.p2p_chat.append({"role": "User", "content": u_input})
                    st.rerun()
            
            with c_status:
                st.markdown(f'<div class="status-card"><b>Ghost Mode:</b> {"ACTIVE" if ghost_active else "Standard"}<br><b>Provider:</b> {doc["name"]}</div>', unsafe_allow_html=True)
                if st.button("Open Video Mesh"):
                    st.info("Sovereign Video Mesh Connecting...")
